...
# عدّله بن غسّان تصليحًا للبرنامج                                        |
# رابع عشر من جمادى الأوّل, 1444 هجريّة
#
#                                                                                    |
# Creation Date:  15 June  2009                                                     |
# Revision Dates: 15 June  2009 - Just finishing off the User Interface.
# Revisions Date: 12 December 2022 - update for Python 3.8 onward.
#                                                                                   |
# ----------------------------------------------------------------------------------|
#                                                                                   |
...
def AboutBox():
    ...
    canvas                 = Canvas( thisroot, width = 500, height = 200 )
    ...
    canvas.create_text( 180, 20, anchor = CENTER, text = 'Version 1.1' )
    canvas.create_text( 180, 40, anchor = CENTER, text = 'Release date: 12 December 2022' )
    canvas.create_text( 180, 60, anchor = CENTER, text = 'Author: GenjamesWolfe' )
...
#Knight Errant had some bizarre command that no longer works for Python after 3.8.
#Therefore, the menu was redesigned. It isn't as pretty, but it's far more likely to last.
#I'll be tinkering with the script to see what I can get out of it, but this is my first foray into
#Python programming (outside of class)
...
#the menu
Cmd_menu = Menu(menubar, tearoff = 0)

#The about
Cmd_menu.add_command(
    label = 'About', command = AboutBox
)
Cmd_menu.add_separator()

#Submenu 1
Sub_Cmd1 = Menu(Cmd_menu, tearoff = 0)

Sub_Cmd1.add_command(
    label = 'Skeleton to text converter', command = SkeletonToTxtConverter
)

Sub_Cmd1.add_command(
    label = 'Text to skeleton converter', command = TxtToSkeletonConverter
)

Cmd_menu.add_command(
    label = 'Info', command = Info_SkeletonToTxtConverter
)

Cmd_menu.add_command(
    label = 'Info', command = Info_TxtToSkeletonConverter
)

Cmd_menu.add_cascade(
label = 'Converter', menu = Sub_Cmd1
)


#Basic Cascade structure:
menubar.add_cascade(
    label = "File", menu = Cmd_menu, underline = 0
)
